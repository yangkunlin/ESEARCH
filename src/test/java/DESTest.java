...
//        System.out.println(new String(DESUtils.decrypt(DESUtils.encrypt(str.getBytes(), "95880288"), "95880288")));
        ...
        ResultSet resultSet = MysqlUtil.getStatement().executeQuery("SELECT videoID, videoName FROM video");

        while (resultSet.next()) {
            System.out.println(resultSet.getString("videoID") + " " + resultSet.getString("videoName"));
        }
        resultSet.close();
        MysqlUtil.close();
    }
