...
public class XiyuanDelIndexC {
    ...
    public String addIndex(@RequestBody String body) throws Exception {
        ...
        String id = searchService.searchForDel(ESParams.XIYUAN_INDEX, ESParams.XIYUAN_TYPE, "gid", bodyJSON.getAsString("gid"));

        if (id.isEmpty()) {
            return "Fail";
        } else {
            boolean isSuccess = indexService.delWithID(ESParams.XIYUAN_INDEX, ESParams.XIYUAN_TYPE, id.replaceAll("\"", ""));
            if (isSuccess) {
                return "Success";
            } else {
                return "Fail";
